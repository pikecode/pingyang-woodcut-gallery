...
项目开发重点已由多端并行调整为 Desktop 优先。H5 和小程序保留现状，暂不继续扩展功能；当前阶段集中解决桌面应用独立运行、离线展示、导览音频和安装包交付问题。
...
assets/originals/{slug}/       438 MB 原图，Git 忽略、仅作为资产处理源
        │
...
- 当前界面不直接读取原始高清图；桌面端运行时只使用 WebP 展示图和离线导览音频。
...
- 移除不再展示的外部原图库选择、原图 IPC 和 TIFF 内存预览逻辑。
- 移除未使用的 Tauri dialog/opener 插件权限，桌面壳只保留默认窗口能力。
- 关闭全局 Tauri API 暴露，并配置最小 CSP，降低桌面 WebView 攻击面。
...
- CI 配置前端生产构建、Rust 单测、macOS DMG 和 Windows MSI/NSIS 构建产物上传，并支持手动创建草稿 Release。
...
| 文件或目录 | 本阶段职责 |
|---|---|
| `desktop/src/App.jsx` | 接入当前开屏、分类与搜索、详情查看器、音频播放器、全屏和键盘交互 |
| `desktop/src/opening/` | 当前长卷开屏、旧门扉备份、独立样式和共享会话控制 |
| `desktop/src/styles.css` | Desktop 宽屏布局、详情查看器、缩放拖拽状态和音频控制条 |
| `desktop/src-tauri/src/lib.rs` | 最小 Tauri 启动入口 |
| `desktop/src-tauri/Cargo.toml` | Tauri 运行和打包依赖 |
| `desktop/src-tauri/capabilities/default.json` | 最小桌面权限配置 |
| `desktop/src-tauri/tauri.conf.json` | 窗口、CSP、安全设置、应用标识、打包图标和 macOS ad-hoc 签名配置 |
...
| `README.md` | Desktop 优先的项目入口、开发命令和资产策略说明 |
| `docs/桌面端规划.md` | 当前技术路线、完成状态和后续功能规划 |
| `docs/桌面端打包与分发说明.md` | 安装包、签名、公证与平台安全提示 |
| `docs/Voicebox导览音频生成与更新手册.md` | 标题与简介旁白规则、增量生成、故障恢复、验收和提交操作手册 |
...
| `cargo test --manifest-path desktop/src-tauri/Cargo.toml` | 通过，当前 Tauri 壳无业务 Rust 单测 |
...
- Windows MSI/NSIS 已配置 CI 构建，但尚未在 Windows 真机完成安装、启动、全屏和音频播放验收。
- macOS DMG 目前是 ad-hoc 签名，公开分发仍会受到 Gatekeeper 提示；正式分发需要 Developer ID 签名和 Apple 公证。
- 原图不进入 Git 和默认安装包。当前桌面端不直接展示原图，后续若恢复该能力，需要重新设计外部资产目录和安全权限。
...
1. 在 GitHub Actions 和 Windows 真机验证 MSI/NSIS 的安装、卸载、首次启动、全屏和音频播放。
2. 确认馆方原始图档的长期外部保存方式，保持后续数据重建能力。
3. 确定 macOS 分发范围：内部使用可继续 ad-hoc；公开分发则配置 Developer ID 签名、公证和 CI secrets。

### P1：桌面工作流

1. 增加原生菜单，将全屏、缩放、退出等高频命令纳入系统菜单。
...
2. 原始图档是否仅作为数据处理源保存，还是后续恢复桌面端高清原图查看。
...
本阶段结论：Desktop 已具备独立离线浏览、桌面看图交互、离线导览音频和 macOS 安装包构建能力。下一阶段不宜继续扩展 H5 或小程序，应先完成跨平台安装验收与正式分发链路，再进入打印和展览模式开发。
