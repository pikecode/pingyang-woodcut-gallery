    ...
    <header className="site-header">
      <div className="header-inner">
        <Link className="brand" to="/" aria-label="平阳木版年画首页">
          ...
          <span><strong>平阳木版年画</strong><small>数字馆藏</small></span>
        </Link>
        <nav className={open ? "main-nav is-open" : "main-nav"} aria-label="主导航">
          <NavLink to="/" end>首页</NavLink>
          <NavLink to="/categories">藏品</NavLink>
          <NavLink to="/search">搜索</NavLink>
        </nav>
        <div className="header-actions">
          <button className="icon-button" type="button" aria-label="搜索" onClick={() => navigate("/search")}>
            <Search size={19} />
          </button>
          <button className="icon-button menu-button" type="button" aria-label={open ? "关闭" : "菜单"}
